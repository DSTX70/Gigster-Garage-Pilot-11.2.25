...
async function verifyHttpLocal(args: { urlPath: string; expectStatus?: number; expectContains?: string; timeoutMs?: number }) {
  ...
  const expectStatus = typeof args.expectStatus === "number" && !Number.isNaN(args.expectStatus) ? args.expectStatus : 200;
  const expectContains = args.expectContains || "";
  const timeoutMs = typeof args.timeoutMs === "number" && !Number.isNaN(args.timeoutMs) ? args.timeoutMs : 4000;
...
function verifyFile(args: { filePath: string; expectContains?: string; maxBytes?: number }) {
  ...
  const expectContains = args.expectContains || "";
  const maxBytes = typeof args.maxBytes === "number" && !Number.isNaN(args.maxBytes) ? args.maxBytes : 65536;
