...
export async function generateProposalPDF(proposal: any): Promise<Buffer> {
  ...
  if (proposal.content) {
    ...
    doc.fontSize(10).fillColor(COLORS.text);
    const contentLines = escapeText(proposal.content).split('\n');
    contentLines.forEach(line => {
      doc.text(line, 50, currentY, { width: doc.page.width - 100 });
      currentY += 15;
    });
  }
  ...
  doc.fontSize(9).fillColor(COLORS.lightGray);
  doc.text('Gigster Garage - Professional Project Management & Client Collaboration Platform', 50, pageHeight - 50, { align: 'center', width: doc.page.width - 100 });
  doc.text(`Generated on ${formatDate(new Date())}`, 50, pageHeight - 38, { align: 'center', width: doc.page.width - 100 });
...
export async function generateContractPDF(contract: any): Promise<Buffer> {
  ...
  sections.forEach(section => {
    if (section.content) {
      ...
      doc.fontSize(10).fillColor(COLORS.text);
      const lines = escapeText(section.content).split('\n');
      lines.forEach(line => {
  ...
  const pageHeight = doc.page.height;
  doc.fontSize(9).fillColor(COLORS.lightGray);
  doc.text('This contract is governed by applicable laws and regulations.', 50, pageHeight - 50, { align: 'center', width: doc.page.width - 100 });
  doc.text('Generated by Gigster Garage - Simplified Workflow Hub', 50, pageHeight - 38, { align: 'center', width: doc.page.width - 100 });
    ...
    slides.forEach((slide: any, index: number) => {
      ...
      if (slide.slideType === 'title' && index === 0) {
        ...
        if (slide.content) {
          ...
          const lines = escapeText(slide.content).split('\n');
          let contentY = 110;
          lines.forEach(line => {
            doc.text(line, 50, contentY, { width: doc.page.width - 100 });
            contentY += 18;
          });
        }
      }
