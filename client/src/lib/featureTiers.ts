...
export function resolvePlanFeatures(
  ...
  const packs: TierFeature[] = [];
  
  const add = (slug: string) => { 
    const p = tiers.tiers.expansion_packs.find(x => x.slug === slug); 
    if (p) {
      const features = p.features.map(f => ({ ...f, source: slug }));
      packs.push(...features);
    }
  };
  ...
  if (plan === "pro") { 
    add("proposals_ai"); 
    add("client_portal"); 
    add("analytics_insights"); 
    add("automation_power"); 
  }
  
  const allowed = (f: TierFeature) => flags[`feature.${f.key}`] !== false;
  return { 
    core: core.filter(allowed), 
    packs: packs.filter(allowed) 
  };
}
