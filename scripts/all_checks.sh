...
# Usage:
#   scripts/all_checks.sh [run|stop] [--fast] [--only <suite>] [--no-spawn] [--no-ping] [--no-tests]
#     suite options: schema | rl | ratelimits | social | sanity | <glob-prefix>
#
# Examples:
#   scripts/all_checks.sh                           # full run (spawn + migrations + tests + pings)
#   scripts/all_checks.sh --no-tests                # just spawn + migrations + pings (no Jest)
#   scripts/all_checks.sh --no-spawn --no-ping      # assume services up; skip endpoint checks
#   scripts/all_checks.sh --fast --only social      # skip migrations; run only social tests
#   scripts/all_checks.sh stop                      # stop spawned server/worker
...
run_tests(){
  ...
  if [[ "$NO_TESTS" -eq 1 ]]; then
    warn "--no-tests → skipping Jest suites"
    ((SKIP++)); return
  fi
...
ping_endpoints(){
  ...
  if [[ "$NO_PING" -eq 1 ]]; then
    warn "--no-ping → skipping endpoint checks"
    ((SKIP+=3)); return
  fi
