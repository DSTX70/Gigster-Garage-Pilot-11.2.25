...
-   **User Management**: Multi-user authentication, role-based access (admin/user/tester), guided onboarding system (under 10 minutes setup), and an admin dashboard.
-   **Tester Login**: Special access mode for QA testers with full platform access to all features without plan gating. Requires `TESTER_ACCESS_CODE` environment variable. Access via "Tester Login" button on login page. Creates a `platform_tester` user with role `tester` and bypasses all feature restrictions.
...
-   **Business Profile Onboarding Wizard** (`/onboarding-wizard`): Comprehensive 6-step wizard for AI Coach personalization collecting role, goals, business basics, stage, offerings, workflow, and pain points. Features 650ms debounced autosave, resume support, and chip-based multi-select inputs. Profile data is used by GigsterCoach for personalized suggestions. Data stored in `user_profiles`, `business_profiles`, and `onboarding_progress` database tables. Post-wizard editing available at `/settings/profile`. **Enhanced session handling (Jan 2026)**: localStorage backup saves progress immediately on every change, session keep-alive pings every 20 seconds to detect expiration, friendly "Session expired" banner with "Log back in" button that preserves all entered data, and smart merge logic that prefers recent local data when resuming.
